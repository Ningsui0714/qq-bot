...
一个基于 C++ 和 WebSocket 的 QQ 群聊机器人，支持多种交互功能。

## 功能特性

- 🎮 **猜数游戏**：@机器人发送"猜数"启动1-100猜数字游戏
- 📚 **课表管理**：支持课表的导入、查询和清空功能
- 💬 **自动回复**：支持关键词匹配自动回复
...
- **语言**：C++17
- **网络**：Boost.Asio + Boost.Beast（WebSocket）
- **JSON解析**：nlohmann/json
- **编译环境**：Visual Studio 2019/2022（Windows）

## 依赖项

- [Boost](https://www.boost.org/) - 用于网络通信
- [nlohmann/json](https://github.com/nlohmann/json) - JSON解析库
- [NapCat](https://github.com/NapNeko/NapCatQQ) - QQ协议实现
...
// 机器人配置
const char* const BOT_QQ = "你的机器人QQ号";
```
...
2. 安装 vcpkg 并配置 Boost 和 nlohmann-json
   ```bash
...
2. 使用 Visual Studio 打开 `qq-bot/qq-bot.sln`
...
4. 选择 Release/x64 配置，编译项目

### 运行

1. 确保 NapCat 已启动并监听 WebSocket 端口
2. 运行编译后的 `qq-bot.exe`
...
在 `reply_generator.cpp` 中添加新规则：

```cpp
{
    [](const json& msg_data, const std::string& content) {
        return is_at_bot(msg_data) && content == "你的关键词";
    },
...
1. 创建新的 `.cpp/.h` 文件
2. 实现 `std::vector<ReplyRule> get_xxx_rules()` 函数
3. 在 `msg_handler.cpp` 中调用规则集
...
- [NapCat](https://github.com/NapNeko/NapCatQQ) - QQ协议支持
- [Boost](https://www.boost.org/) - 网络库
